...
{strip}
    {foreach from=$providers_list item="provider_data"}
        ...
        <a class="cm-login-provider ty-hybrid-auth__icon" data-idp="{$provider_data.provider}"><img src="{$images_dir}/addons/hybrid_auth/icons/{$addons.hybrid_auth.icons_pack}/{$provider_data.provider}.png" title="{$provider_data.provider}" /></a>
        {/if}
    {/foreach}
{/strip}
{/capture}
