...
<tr>
    ...
    {/if}
        {if !$gift.extra.exclude_from_calculate}
            <a href="{"gift_certificates.update?gift_cert_id=`$gift_key`"|fn_url}" class="product-title">{__("gift_certificate")}{if !$gift.extra.exclude_from_calculate}<a class="{$ajax_class} cm-post icon-delete-big" href="{"gift_certificates.delete?gift_cert_id=`$gift_key`&redirect_url=`$c_url`"|fn_url}"  data-ca-target-id="cart_items,checkout_totals,cart_status*,checkout_steps,checkout_cart" title="{__("remove")}"><i class="icon-cancel-circle"></i></a>{/if}</a>&nbsp;
