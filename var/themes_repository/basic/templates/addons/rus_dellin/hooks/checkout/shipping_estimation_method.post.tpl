            ...
            <div class="checkout-select-terminals">
                {foreach from=$shipping.data.arrival_terminals item=arrival_terminal}
                    ...
                    <div class="one-terminal">
                        <input type="radio" name="arrival_terminal[{$group_key}][{$shipping.shipping_id}]" value="{$arrival_terminal.code}" {if $old_terminal == $arrival_terminal.code || $arrival_count == 1}checked="checked"{/if} id="office_{$arrival_terminal.code}" class="terminal-radio" />
