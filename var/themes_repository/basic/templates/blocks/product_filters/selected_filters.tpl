...
{foreach from=$items item="filter"}
    ...
    <div class="dropdown-container">
        ...
        <span id="sw_elm_selected_filter_{$filter.filter_id}" class="sort-dropdown cm-combination"><a>{$filter.filter}{if $filter.selected_variants} ({$filter.selected_variants|sizeof}){/if}<a class="icon-cancel-circle cm-ajax cm-ajax-full-render cm-history" href="{$reset_url|fn_url}" data-ca-event="ce.filtersinit" data-ca-target-id="{$ajax_div_ids}" data-ca-scroll=".cm-pagination-container"></a><i class="icon-down-micro"></i></a></span>

        <div id="elm_selected_filter_{$filter.filter_id}" class="cm-popup-box hidden dropdown-content filters-dropdown-content">
