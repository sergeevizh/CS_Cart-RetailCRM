<div class="step-container{if $edit}-active{/if} step-two" data-ct-checkout="billing_shipping_address" id="step_two">
    <h2 class="step-title{if $edit}-active{/if}{if $complete && !$edit}-complete{/if} clearfix">
        ...
        {if ($show_number_of_step && $number_of_step) || !$show_number_of_step}
            <span class="float-left">{if $show_number_of_step}{$number_of_step}{else}<i class="icon-ok"></i>{/if}</span>
        {/if}

        {if $complete && !$edit}
            {hook name="checkout:step_two_edit_link"}
            <span class="float-right">
                {include file="buttons/button.tpl" but_meta="cm-ajax" but_href="checkout.checkout?edit_step=step_two&from_step={$cart.edit_step}" but_target_id="checkout_*" but_text=__("change") but_role="tool"}
            </span>
            {/hook}
        {/if}
        
        {hook name="checkout:step_two_edit_link_title"}
        <a class="title{if $complete && !$edit} cm-ajax{/if}" {if $complete && !$edit}href="{"checkout.checkout?edit_step=step_two&from_step=`$cart.edit_step`"|fn_url}" data-ca-target-id="checkout_*"{/if}>{__("billing_shipping_address")}</a>
        {/hook}
    </h2>
            ...
            {if $edit}
                ...
                {else}
                    <div class="checkout-buttons">
                        {include file="buttons/button.tpl" but_name="dispatch[checkout.update_steps]" but_text=__("continue")}
                    </div>
                {/if}
            {/if}
