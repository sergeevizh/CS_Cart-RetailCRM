...
{capture name="name_`$obj_id`"}
    {if $show_name}
        {if $hide_links}<strong>{else}<a href="{"companies.products?company_id=`$company.company_id`"|fn_url}" class="ty-company-title">{/if}{$company.company nofilter}{if $hide_links}</strong>{else}</a>{/if}
    {elseif $show_trunc_name}
        {if $hide_links}<strong>{else}<a href="{"companies.products?company_id=`$company.company_id`"|fn_url}" class="ty-company-title" title="{$company.company|strip_tags}">{/if}{$company.company|truncate:45:"...":true nofilter}{if $hide_links}</strong>{else}</a>{/if}
    {/if}
{/capture}
{if $no_capture}
