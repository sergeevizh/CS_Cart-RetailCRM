(function(_, $){
    ...
    $(_.doc).on('click', function (e) {
        ...
        if ($.matchClass(elm, /cm-spsr_form_call(-[\w]+)?/gi)) {

            var p_elm = (jelm.parents('.cm-spsr_form_call').length) ? jelm.parents('.cm-spsr_form_call:first') : (jelm.prop('id') ? jelm : jelm.parent());
            var id, prefix;
            if (p_elm.prop('id')) {
