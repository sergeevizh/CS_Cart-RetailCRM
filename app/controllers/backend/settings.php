    ...
    if ($mode == 'change_store_mode') {
        if ($_REQUEST['store_mode'] == 'full') {
            if (empty($_REQUEST['license_number'])) {
                ...
                $trial_expired = fn_get_storage_data('store_mode_trial') === 'trial_is_expired';
                $data = Helpdesk::getLicenseInformation($_REQUEST['license_number'], array('store_mode_selector' => 'Y'));
                list($license_status, $updates, $server_messages) = Helpdesk::parseLicenseInformation($data, $auth, false);
                ...
                if ($license_status == 'ACTIVE') {
                    ...
                    switch ($license_status) {
                        ...
                        case 'SUSPENDED':
                        case 'DISABLED':
                            $messages['license_is_disabled'] = array(
                                'type' => 'E',
                                'title' => __('error'),
                                'text' => $trial_expired ? __('text_license_is_disabled') : __('text_license_is_disabled_full')
                            );
                            break;
                        case 'LICENSE_IS_INVALID':
                            $messages['license_is_invalid'] = array(
                                'type' => 'E',
                                'title' => __('error'),
                                'text' => $trial_expired ? __('text_license_is_invalid') : __('text_license_is_invalid_full')
                            );
                            break;
                        default:
        ...
        } elseif ($_REQUEST['store_mode'] == 'trial') {
