...
class Ups implements IService
{
    ...
    public function getRequestData()
    {
        ...
        if ($origination['country'] == 'US' && $pickup_type == '11') {
...
EOT;
        ...
        foreach ($this->_shipping_info['package_info']['packages'] as $package) {
            ...
            $weight = (!in_array($origination['country'], array('US', 'DO','PR'))) ? ($weight_ar['full_pounds'] * 0.4536) : $weight_ar['full_pounds'];
...
EOT;
    ...
    <RatingServiceSelectionRequest xml:lang="en-US">
      ...
      <Shipment>
        <Shipper>
            <Address>
                <PostalCode>{$origination['zipcode']}</PostalCode>
                <CountryCode>{$origination['country']}</CountryCode>
            </Address>
            $shipper
        </Shipper>
        <ShipTo>
            <Address>
                <StateProvinceCode>{$location['state']}</StateProvinceCode>
        ...
        <ShipFrom>
            <Address>
                <StateProvinceCode>{$origination['state']}</StateProvinceCode>
                <PostalCode>{$origination['zipcode']}</PostalCode>
                <CountryCode>{$origination['country']}</CountryCode>
                <ResidentialAddressIndicator/>
            </Address>
