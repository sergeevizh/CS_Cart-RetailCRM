...
class Backend extends ACommon
{
    ...
    public function prepare(&$params)
    {
        if (!empty($params['return_to_list']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            } elseif (!empty($this->_schema['update_mode'])
        ...
        if (isset($this->_schema)
            ...
            if (!in_array(Registry::get('runtime.action'), $_actions)
                && (
                    !(Registry::get('runtime.action') == 'last_view' && empty($params['view_id']))
                    || (
                        Registry::get('runtime.action') == 'last_view' && empty($params['view_id'])
