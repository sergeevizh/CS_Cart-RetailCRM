...
function db_export_to_file($file_name, $dbdump_tables, $dbdump_schema, $dbdump_data, $log = true, $show_progress = true, $move_progress_bar = true, $change_table_prefix = array())
{
    ...
    // set export format
    Tygh::$app['db']->query("SET @SQL_MODE = 'MYSQL323'");
    ...
    // get status data
    $t_status = Tygh::$app['db']->getHash("SHOW TABLE STATUS", 'Name');
    ...
    foreach ($dbdump_tables as $k => $table) {
        ...
        if ($dbdump_schema) {
            ...
            $scheme = Tygh::$app['db']->getRow("SHOW CREATE TABLE $table");
        ...
        if ($dbdump_data) {
            ...
            $total_rows = Tygh::$app['db']->getField("SELECT COUNT(*) FROM $table");
            ...
            for ($i = 0; $i < $total_rows; $i = $i + $it) {
                $table_data = Tygh::$app['db']->getArray("SELECT * FROM $table LIMIT $i, $it");
...
function db_paginate(&$page, &$items_per_page, $total_items = 0)
{
    ...
    if ($items_per_page <= 0) {
        $items_per_page = (int) Registry::ifGet('settings.Appearance.elements_per_page', 10);
    }
