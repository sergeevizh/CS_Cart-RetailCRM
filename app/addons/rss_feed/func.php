...
function fn_format_products_items($products, $params = array(), $lang_code = CART_LANGUAGE)
{
    ...
    foreach ($products as $key => $product_data) {
        ...
        $items_data[$key] = array (
            ...
            'link' => fn_url('products.view?product_id=' . $product_data['product_id'], 'C', 'current', $lang_code, true),
...
{
    $product_url = fn_url('products.view?product_id=' . $product_data['product_id'], 'C', 'current', $lang_code, true);
...
EOT;
    ...
    if (!empty($params['rss_display_add_to_cart']) && $params['rss_display_add_to_cart'] == 'Y') {
        ...
        if (Registry::get('settings.General.allow_anonymous_shopping') == 'allow_shopping') {
            $add_to_cart_url = fn_url('rss.add_to_cart?product_id=' . $product_data['product_id'] . '&lang=' . $lang_code, 'C', 'current', $lang_code, true);
