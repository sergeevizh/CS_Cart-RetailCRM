...
function fn_yml_export_update_product_feature_pre(&$feature_data)
{
    ...
    if (!empty($feature_data['yml2_exclude_prices'])) {
        ...
        if ($product_update && !empty($feature['variants'])) {
            foreach ($feature['variants'] as &$variant) {
                if (!empty($variant['yml2_unit'])) {
                    $variant['variant'] .= $variant['yml2_unit'];
                }
            }
        }
        ...
        if (!empty($feature['yml2_variants_unit'])) {
            ...
            if (!empty($feature['variants']) && !empty($feature['variant_id']) && !empty($feature['variants'][$feature['variant_id']])) {
                ...
                if (!empty($variant['yml2_unit']) && !$product_update) {
                    $feature['suffix'] .= $variant['yml2_unit'];
                } else {
                    $feature['suffix'] .= $feature['yml2_variants_unit'];
                }
            } else {
                $feature['suffix'] .= $feature['yml2_variants_unit'];
            }
...
function fn_yml_export_get_product_features_list_post(&$features_list, $product, $display_on, $lang_code)
{
    foreach($features_list as $feature_id => &$feature) {
        ...
        if (!empty($yml_variant_unit)) {
            $feature['suffix'] .= $yml_variant_unit;
        ...
        if (isset($feature['subfeatures'])) {
            fn_yml_export_get_product_features_list_post($feature);
        }
    }
}
...
function fn_yml_export_update_product_feature_variant($feature_id, $feature_type, $variant, $lang_code, &$variant_id)
{
    if (!empty($variant_id)) {
        ...
        $variant_id = db_get_field(
            ...
            . 'WHERE fvd.variant = ?s AND yml2_unit = ?s AND lang_code = ?s', $variant['variant'], $yml2_unit, $lang_code
        );
    }
}
    ...
    if (!empty($delivery_options)) {
        foreach($delivery_options as $option) {
            ...
            if (!empty($option)) {
                ...
                if (isset($option[1])) {
                    $option_data['days'] = (int) trim($option[1]);
                }
...
function fn_yml_get_parent_categories_field($field_name, $category_id, $parent = false)
{
    ...
    if (!empty($path)) {
        $categories_fields = db_get_hash_array("SELECT category_id, $field_name FROM ?:categories WHERE category_id IN (?a) AND $field_name != ''", 'category_id', $path);
...
function fn_yml_export_import_ym($params)
{
    if (!empty($params['import_products']) && $params['import_products'] == 'Y') {
        fn_yml_export_import_products();
    }

    if (!empty($params['import_categories']) && $params['import_categories'] == 'Y') {
        fn_yml_export_import_categories();
    }

    if (!empty($params['import_settings']) && $params['import_settings'] == 'Y') {
        fn_yml_export_import_settings();
    }
}

function fn_yml_export_import_products()
{
    $schema = fn_get_schema('yml', 'import_products');
    ...
    db_query("UPDATE ?:products SET " . implode(', ', $fields));
}

function fn_yml_export_import_categories()
{
    db_query("UPDATE ?:categories SET yml2_market_category = yml_market_category");
}
...
function fn_yml_export_import_settings()
{
    ...
    foreach($companies_ids as $company_id) {
        ...
        $price_params = array(
            'enable_authorization' => fn_yml_get_setting_value('enable_authorization', $company_id),
            'company_id' => $company_id,
            'access_key' => $key,
            'name_price_list' => 'Яндекс.Маркет',
            'shop_name' => $shop_name,
            'export_encoding' => fn_yml_get_setting_value('export_encoding', $company_id),
            'detailed_generation' => 'Y',
            'utm_link' => '',
            'export_stock' => fn_yml_get_setting_value('export_stock', $company_id),
            'export_null_price' => fn_yml_get_setting_value('export_null_price', $company_id),
            'market_category' => fn_yml_get_setting_value('market_category', $company_id),
            'weight' => 'N',
            'dimensions' => 'N',
            'image_type' => fn_yml_get_setting_value('image_type', $company_id),
            'thumbnail_width' => fn_yml_get_setting_value('thumbnail_width', $company_id),
            'thumbnail_height' => fn_yml_get_setting_value('thumbnail_height', $company_id),
            'store' => 'N',
            'pickup' => 'N',
            'delivery' => 'Y',
            'delivery_options' => array(),
            'export_hidden_categories' => 'N',
            'exclude_categories_not_logging' => 'Y',
            'exclude_categories' => '',
        );
        ...
        $data = array(
            'param_type' => 'price_list',
            'param_key' => $key,
            'param_data' => serialize($price_params),
            'company_id' => $company_id
        );
...
function fn_yml_get_setting_value($name, $company_id = 0)
{
    static $section_id = null;
    if (!isset($section_id)) {
        $section_id = db_get_field("SELECT section_id FROM ?:settings_sections WHERE name = 'yandex_market' AND type='ADDON'");
    }

    $setting = db_get_row("SELECT `object_id`, `value` FROM ?:settings_objects WHERE name = ?s AND section_id = ?i", $name, $section_id);
    if (!empty($company_id)) {
        $vendor_setting = db_get_row("SELECT `object_id`, `value` FROM ?:settings_vendor_values WHERE object_id = ?i AND company_id = ?i", $setting['object_id'], $company_id);
