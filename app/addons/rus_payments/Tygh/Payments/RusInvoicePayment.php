...
namespace Tygh\Payments;

class RusInvoicePayment
{
    public $def = array(
    ...
    public static function clearDoit($str) {
        $num2rub = new RusInvoicePayment();
        $rank = array(
            0 => __("addons.rus_payments.rub_rank"),
            1 => __("addons.rus_payments.thousand_rank"),
            2 => __("addons.rus_payments.million_rank"),
            3 => __("addons.rus_payments.billion_rank"),
            'k' => __("addons.rus_payments.kopeck_rank"),
        );
        ...
        $word = array();
        $word[] = __("addons.rus_payments.kopeck_rank", array($kop));
        ...
        foreach($rub as $key => $value) {
            if (intval($value) > 0 || $key == 0) {
                ...
                $str_rank = str_replace("_", "", $str_rank);
