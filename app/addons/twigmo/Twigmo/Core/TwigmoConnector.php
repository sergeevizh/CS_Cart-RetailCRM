...
class TwigmoConnector extends \AbstractTwigmoConnector
{
    ...
    const MAX_PUSH_LENGTH =     500; // Symbols
    ...
    public function authPage($page)
    {
        ...
        fn_redirect($url, true, PRODUCT_VERSION == '4.0.1');  // $allow_external_redirect is the third param in 4.0.1
    }
            ...
            if (!empty($store_plans)) {
                foreach($stores as &$store) {
                    if (isset($store['access_id']) && isset($store_plans[$store['access_id']])) {
                        $store_svc_info = $store_plans[$store['access_id']];
                        $store['plan_display_name'] = $store_svc_info['plan_display_name'];
                        $store['push_subscribers_ios'] = $store_svc_info['push_subscribers']['ios'];
                        $store['push_subscribers_android'] = $store_svc_info['push_subscribers']['android'];
                        TwigmoSettings::set(array('customer_connections' => array($store['company_id'] => $store)));
                    } elseif (isset($store['access_id']) && !isset($store_plans[$store['access_id']])) {
                        self::disconnect(array($store['company_id']), $disconnect_all = false);
                    }
                }
                unset($store);
            }
    ...
    public static function getAdminUrl($override_area = true)
    {
        $secure_admin = Settings::instance()->getSettingDataByName('secure_admin');
        return fn_url(self::DISPATCH, 'A', $secure_admin['value'] == 'Y' ? 'https' : 'http', CART_LANGUAGE, $override_area);
    }
    ...
    public static function getMobileScriptsUrls($base_url = '')
    {
        ...
        if ($favicon_url) {
            $urls['favicon'] = preg_replace('/^https?:/', '', $favicon_url);
        } else {
            $urls['favicon'] = $root_url . self::FAVICON_PATH;
        }
    ...
    public static function disconnect($company_ids, $disconnect_all)
    {
        ...
        $disconnected_store = array('access_id' => '', 'plan_display_name' => '');
