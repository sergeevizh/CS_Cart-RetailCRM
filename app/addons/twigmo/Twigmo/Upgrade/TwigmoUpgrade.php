...
class TwigmoUpgrade
{
    ...
    private static $file_areas_admin = array(
        'media' =>      'media/images',
        'css' =>        'css',
        'templates' =>  'templates'
    );
    private static $file_areas_customer = array(
        'css' =>        'css',
        'templates' =>  'templates'
    );
    ...
    public static function getUpgradeDirs($install_src_dir)
    {
        ...
        if (fn_allowed_for('ULTIMATE')) {
            ...
            foreach (array_keys(self::$file_areas_customer) as $key) {
    ...
    private static function setInstalledDirs()
    {
        ...
        foreach (self::$file_areas_admin as $key => $file_area) {
            self::$dirs['installed'][$key . '_backend'] = fn_get_theme_path('[themes]/[theme]/', 'A') . $file_area . '/' . self::$full_addon_path;
        }
        foreach (self::$file_areas_customer as $key => $file_area) {
            foreach (self::$installed_themes as $theme) {
                self::$dirs['installed'][$key . '_frontend'][0][$theme] = fn_get_theme_path('[themes]/' . $theme, 'C') . '/' . $file_area . '/' . self::$full_addon_path;
            }
        }
    ...
    private static function setRepoDirs()
    {
        ...
        foreach (self::$file_areas_admin as $key => $file_area) {
            self::$dirs['repo'][$key . '_backend'] = fn_get_theme_path('[themes]/', 'A') . $file_area . '/' . self::$full_addon_path;
        }
        foreach (self::$file_areas_customer as $key => $file_area) {
            foreach (self::$repo_themes as $theme) {
                self::$dirs['repo'][$key . '_frontend'][$theme] = fn_get_theme_path('[repo]/' . $theme, 'C') . '/' . $file_area . '/' . self::$full_addon_path;
            }
        }
    ...
    private static function setDistrDirs()
    {
        ...
        foreach (self::$file_areas_admin as $key => $file_area) {
            self::$dirs['distr'][$key . '_backend'] = self::$install_src_dir . 'design/backend/' . $file_area . '/' . self::$full_addon_path;
        }
        foreach (self::$file_areas_customer as $key => $file_area) {
            foreach (self::$repo_themes as $theme) {
                self::$dirs['distr'][$key . '_frontend'][$theme] = self::$install_src_dir . self::$repo_path . $file_area . '/' . self::$full_addon_path;
            }
        }
    }
    ...
    private static function setBackupDirs()
    {
        ...
        foreach (self::$file_areas_admin as $key => $file_area) {
            self::$dirs['backup_files'][$key . '_backend'] = self::$dirs['backup_root'] . self::$backup_files_path . fn_get_theme_path('[relative]/[theme]/', 'A') . $file_area . '/' . self::$full_addon_path;
        }
        foreach (self::$file_areas_customer as $key => $file_area) {
            foreach (self::$installed_themes as $theme) {
                self::$dirs['backup_files'][$key . '_frontend'][0][$theme] = self::$dirs['backup_root'] . self::$backup_files_path . fn_get_theme_path('[relative]/' . $theme . '/', 'C') . $file_area . '/'  . self::$full_addon_path;
            }
        }
    ...
    private static function setInstalledFrontend($company_id)
    {
        foreach (self::$file_areas_customer as $key => $file_area) {
            foreach (self::$installed_themes as $theme) {
                self::$dirs['installed'][$key . '_frontend'][$company_id][$theme] = fn_get_theme_path('[themes]/' . $theme, 'C', $company_id) . '/' . $file_area . '/' . self::$full_addon_path;
            }
        }
