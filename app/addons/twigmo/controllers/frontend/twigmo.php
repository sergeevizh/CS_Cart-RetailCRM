...
if (($_SERVER['REQUEST_METHOD'] == 'POST' || $is_cache_request) && $mode == 'post') {
    ...
    if ($meta['action'] == 'login') {
        ...
        if (version_compare(PRODUCT_VERSION, '4.2.3', '>=')) {
            $cart['pending_coupon'] = strtolower($cart['pending_coupon']);
        }
        $cart['recalculate'] = true;
        ...
        if ($meta['object'] == 'cart') {
            ...
            $user_data['copy_address'] = !empty($user_data['copy_address']);
            ...
            if (!empty($user_data['copy_address'])) {
                $profile_fields = fn_get_profile_fields('O');
        ...
        if ($meta['object'] == 'page') {
            ...
            if (!empty($payment_methods['payment'])) {
                foreach ($payment_methods['payment'] as $k => $payment_method) {
                    $options = fn_twg_get_payment_options($payment_method);
                    if ($options) {
        ...
        if ($meta['object'] == 'products') {
            $_REQUEST['product_id'] = $_REQUEST['id'];
            Registry::set('runtime.checkout', false);
    ...
    } elseif ($meta['action'] == 'send_order_push') {
        fn_twg_send_notification();
...
function fn_twg_send_notification()
{
    if (!empty($_SESSION['twg_state']['send_push_for_order_ids'])) {
        $connector = new TwigmoConnector();
        fn_twg_send_order_push_notification($_SESSION['twg_state']['send_push_for_order_ids'], $connector);
