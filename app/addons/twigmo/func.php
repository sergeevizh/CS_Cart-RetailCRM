...
function fn_twg_assign_google_template()
{
    ...
    foreach ($google_templates as $google_template) {
        if ($view->templateExists($google_template)) {
            ...
            $view->assign('do_not_use_google_template', true);
            return;
        }
    }
}

function fn_twg_get_lang_var($var_name, $lang_code = CART_LANGUAGE, $params = array())
{
    // Order params changed
    return __($var_name, $params, $lang_code);
}
...
function fn_twg_get_view_object()
{
...
function fn_twigmo_get_users(&$params, &$fields, &$sortings, &$condition, &$join)
{
    ...
    if ($twg_condition) {
        $condition['twg_users'] = $twg_condition;
    }
}
...
function fn_twigmo_place_order($order_id, $action = '', $__order_status = '', $cart = null)  // Hook
{
    ...
    if (empty($_SESSION['twg_state']['send_push_for_order_ids'])) {
        $_SESSION['twg_state']['send_push_for_order_ids'] = array();
    }
    $_SESSION['twg_state']['send_push_for_order_ids'][] = $order_id;
}

function fn_twigmo_order_notification($order_info, $order_statuses, $force_notification)  // Hook
{
    $connector = new TwigmoConnector();
    $company_id = fn_allowed_for('ULTIMATE') ? $order_info['company_id'] : 0;
...
function fn_twg_use_https_for_customer($company_id = 0)
{
    ...
    if (function_exists('fn_get_storefront_protocol')) {
        // For 4.3+
        $secure_storefront = $settings_object->getSettingDataByName('secure_storefront', $company_id);
        $is_secure_storefront = $secure_storefront['value'] != 'none';
...
function fn_twg_api_get_products($params, $items_per_page = 10, $lang_code = CART_LANGUAGE)
{
    ...
    foreach ($products as $key => $product) {
        ...
        if (!empty($product['main_pair'])) {
            $products[$key]['icon'] = TwigmoImage::getApiImageData($product['main_pair'], 'product', 'icon', $image_params);
        }
...
function fn_twg_get_api_product_data($product_id, $lang_code = CART_LANGUAGE)
{
    ...
    if (Registry::get('addons.discussion.status') == 'A') {
        $discussion = fn_get_discussion($product_id, 'P', true);
        if (!empty($discussion['thread_id']) && !empty($discussion['posts'])) {
            $product['product_reviews'] = $discussion['posts'];
        }
    }
    ...
    $_product = Api::getAsApiObject('product', $product);
...
function fn_twg_get_product_tabs($params)
{
    ...
    $allowed_templates = array(
        'blocks/product_tabs/features.tpl',
        'blocks/product_tabs/description.tpl',
        'addons/discussion/blocks/product_tabs/discussion.tpl'
    );
    ...
    foreach ($tabs as $k => $tab) {
        ...
        if ($tab['tab_type'] == 'B') {
            ...
            if ($tab['template'] == 'addons/discussion/blocks/product_tabs/discussion.tpl') {
                $tabs[$k]['type'] = 'reviews';
            }
...
function fn_twg_get_payment_methods()
{
    ...
    foreach ($payment_methods as $key => $payment_method) {
        ...
        // Get Yandex Money params
        if (!empty($payment_method['template']) && strpos($payment_method['template'], 'yandex_money.tpl') !== false) {
            $ya_params = unserialize($payment_method['processor_params']);
            if (!empty($ya_params['payments'])) {
                $payment_methods[$key]['params'] = $ya_params['payments'];
...
function fn_twg_set_response_products(&$response, $params, $items_per_page = 0, $lang_code = CART_LANGUAGE) {
    ...
    $pagination_params = array(
        'items_per_page' => !empty($params['items_per_page']) ? $params['items_per_page'] : TWG_RESPONSE_ITEMS_LIMIT,
        'page' => !empty($params['page']) ? $params['page'] : 1,
        'total_items' =>  !empty($request['total_items']) ? $request['total_items'] : 0
    );
...
function fn_twg_get_user_info($user_id)
{
    ...
    $profile_pfields = fn_get_profile_fields();

    $profile['ship_to_another']['profile'] = fn_check_shipping_billing($profile, $profile_pfields);
    $profile['ship_to_another']['cart'] = fn_check_shipping_billing($profile, $checkout_pfields);

    $profile['address_flag']['profile'] = fn_compare_shipping_billing($profile_pfields);
    $profile['address_flag']['cart'] = fn_compare_shipping_billing($checkout_pfields);
...
function fn_twg_get_all_settings()
{
    ...
    $settings['display_supplier'] = Registry::get('settings.Suppliers.display_supplier') === 'Y' ||
                                    Registry::get('settings.Vendors.display_vendor') === 'Y';
    ...
    if (Registry::get('addons.google_analytics.status') == 'A') {
        $settings['GATrackEcommerce'] = Registry::get('addons.google_analytics.track_ecommerce') == 'Y'
            && file_exists(Registry::get('config.dir.addons') . 'google_analytics/controllers/frontend/checkout.post.php');
        $settings['GATrackingCode'] = Registry::get('addons.google_analytics.tracking_code');
    }
    ...
    $view = Registry::get('view');
    if ($view->getTemplateVars('searchanise_import_status') == 'done') {
        $settings['searchanise_api_key'] = $view->getTemplateVars('searchanise_api_key');
    }
...
// To avoid errors after upgrade to 3.9
