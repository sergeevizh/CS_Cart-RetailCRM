...
function fn_exim_set_product_features($product_id, $data, $features_delimiter, $lang_code, $store_name = '')
{
    ...
    if (!fn_is_empty($features)) {
        $features = fn_exim_parse_data($features, $features_delimiter);
        $company_id = 0;
        ...
        foreach ($features as $feature) {
            ...
            $feature_id = fn_exim_find_feature_id($feature['name'], $feature['type'], $group_id, $main_lang);
            ...
            if (empty($feature_id)) {
                ...
                $feature_id = fn_update_product_feature($feature_data, 0, $main_lang);
            }
            ...
            $variants = fn_exim_product_feature_variants($feature, $feature_id, $variants, $main_lang);
        }

        fn_update_product_features_value($product_id, $variants, array(), $main_lang);

        foreach ($data as $lang => $features) {
            $text_variants = array();
            $features = fn_exim_parse_data($features, $features_delimiter);

            foreach ($features as $feature) {
                if ($feature['type'] != ProductFeatures::TEXT_FIELD) {
                    continue;
                }
                ...
                if (!empty($feature['group_name'])) {
                    $group_id = fn_exim_find_feature_group_id($feature['group_name'], $lang);
                ...
                $feature_id = fn_exim_find_feature_id($feature['name'], $feature['type'], $group_id, $lang);

                if ($feature_id) {
                    $feature_variant = reset($feature['variants']);

                    if (!empty($feature_variant)) {
                        $text_variants[$feature_id] = $feature_variant['name'];
                    }
                }
            }

            if (!empty($text_variants)) {
                fn_update_product_features_value($product_id, $text_variants, array(), $lang);
            }
        }
    }
 ...
 * @return int
 */
function fn_exim_find_feature_group_id($name, $lang_code)
{
    return (int) db_get_field(
        "SELECT feature_id FROM ?:product_features_descriptions WHERE description = ?s AND lang_code = ?s LIMIT 1",
        $name,
        $lang_code
    );
}
 ...
 * @return int
 */
function fn_exim_find_feature_id($name, $type, $group_id, $lang_code)
{
