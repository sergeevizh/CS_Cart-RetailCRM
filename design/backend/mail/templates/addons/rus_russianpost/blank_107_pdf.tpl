...
<body style="width: 293mm; height: 205mm;">
    <div style="top: {$addons.rus_russianpost.107_top1}mm; left: {$addons.rus_russianpost.107_left1}mm; width: {$addons.rus_russianpost.107_width}mm; height: {$addons.rus_russianpost.107_height}mm; position: relative;" >
        {if $data.print_bg == 'Y'}
        ...
        {foreach from=$order_info.products item=product}
            ...
            <span style="position: absolute; height: 5mm; width: 63mm; top: {$p_size+51.5}mm; left: 172.5mm; font: 6pt 'Arial';">{$product.product}</span>
            ...
            {if ($p_number - ($count_page * 14)) == 1}
                ...
                {/if}
                {$p_size = $p_size + 128}
                <span style="position: absolute; height: 10mm; width: 80mm; top: {$p_size+141.5}mm; left: 20mm; font: 11pt 'Arial';">{if $data.sender == '1'}{$data.whom} {$data.whom2}{else}{$data.fio} {$data.fio2}{/if}</span>
                <span style="position: absolute; height: 10mm; width: 80mm; top: {$p_size+141.5}mm; left: 165mm; font: 11pt 'Arial';">{if $data.sender == '1'}{$data.whom} {$data.whom2}{else}{$data.fio} {$data.fio2}{/if}</span>
