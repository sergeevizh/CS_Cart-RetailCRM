...
{include file="common/subheader.tpl" title=__("rus_pickup.display")}

<div class="control-group">
    <label class="control-label" for="display">{__("rus_pickup.display")}:</label>
    <div class="controls">
        <select name="shipping_data[service_params][display]" id="display">
            <option value="ML" {if $shipping.service_params.display == "ML"}selected="selected"{/if}>{__("rus_pickup.display_ml")}</option>
            <option value="M" {if $shipping.service_params.display == "M"}selected="selected"{/if}>{__("rus_pickup.display_m")}</option>
            <option value="L" {if $shipping.service_params.display == "L"}selected="selected"{/if}>{__("rus_pickup.display_l")}</option>
        </select>
    </div>
</div>
