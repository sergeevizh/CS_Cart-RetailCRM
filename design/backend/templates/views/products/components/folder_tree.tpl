        ...
        <div class="pull-right hidden-tools">
            {capture name="items_tools"}
                ...
                {if !$non_editable && !$skip_delete}
                    <li>{btn type="text" text=__("delete") href={"products.delete_folder?folder_id=`$folder.folder_id`&product_id=`$product_data.product_id`"|fn_url} class="cm-confirm cm-tooltip cm-ajax cm-ajax-force cm-ajax-full-render cm-delete-row" data=["data-ca-target-id" => "product_files_list"] method="POST"}</li>
                {/if}
            {/capture}
