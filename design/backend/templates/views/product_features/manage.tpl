...
<tbody>
    {if $has_ungroupped}
        {foreach from=$features item="p_feature"}
            {if $p_feature.feature_type != "ProductFeatures::GROUP"|enum}
                ...
                {if $p_feature|fn_allow_save_object:"product_features"}
                    {include file="common/object_group.tpl" id=$p_feature.feature_id details=$p_feature.feature_description text=$p_feature.description status=$p_feature.status hidden=true href="product_features.update?feature_id=`$p_feature.feature_id`{if !$show_in_popup}&return_url=`$r_url`{/if}" object_id_name="feature_id" table="product_features" href_delete="product_features.delete?feature_id=`$p_feature.feature_id`&return_url=$r_url" delete_target_id="pagination_contents" header_text="{__("editing_product_feature")}: `$p_feature.description`" update_controller="product_features" no_table=true no_popup=$show_in_popup company_object=$p_feature}
                {else}
                    {include file="common/object_group.tpl" id=$p_feature.feature_id details=$p_feature.feature_description text=$p_feature.description status=$p_feature.status hidden=true href="product_features.update?feature_id=`$p_feature.feature_id`{if !$show_in_popup}&return_url=`$r_url`{/if}" object_id_name="feature_id" table="product_features" additional_class="cm-hide-inputs" header_text="{__("viewing_feature")}: `$p_feature.description`" update_controller="product_features" no_table=true non_editable=true link_text=__("view") is_view_link=true company_object=$p_feature}
                {/if}
            {/if}
        {/foreach}
    {/if}
    ...
    {foreach from=$features item="gr_feature"}
        {if $gr_feature.feature_type == "ProductFeatures::GROUP"|enum}
            ...
            {else}
                {include file="common/object_group.tpl" id=$gr_feature.feature_id details=$gr_feature.feature_description text=$gr_feature.description status=$gr_feature.status hidden=true href="product_features.update?feature_id=`$gr_feature.feature_id`&return_url=$r_url" object_id_name="feature_id" table="product_features" href_delete="product_features.delete?feature_id=`$gr_feature.feature_id`&return_url=$r_url" delete_target_id="pagination_contents,content_add_new_feature" header_text="{__("editing_group")}: `$gr_feature.description`" no_table=true link_meta="strong" company_object=$gr_feature additional_class="row-gray"}
            {/if}
            ...
            {if $gr_feature.subfeatures}
                {foreach from=$gr_feature.subfeatures item="subfeature"}
                    ...
                    {if $subfeature.feature_type == "ProductFeatures::EXTENDED"|enum}
                        {$show_in_popup = true}
                    {else}
                        {$show_in_popup = false}
                    {/if}
                    ...
                    {if !$subfeature|fn_allow_save_object:"product_features"}
                        {include file="common/object_group.tpl" id=$subfeature.feature_id details=$subfeature.feature_description text=$subfeature.description status=$subfeature.status hidden=true href="product_features.update?feature_id=`$subfeature.feature_id`{if !$show_in_popup}&return_url=`$r_url`{/if}" object_id_name="feature_id" table="product_features" additional_class="cm-hide-inputs" header_text="{__("viewing_feature")}: `$subfeature.description`"
                    ...
                    {else}
                        {include file="common/object_group.tpl" id=$subfeature.feature_id details=$subfeature.feature_description text=$subfeature.description status=$subfeature.status hidden=true href="product_features.update?feature_id=`$subfeature.feature_id`{if !$show_in_popup}&return_url=`$r_url`{/if}" object_id_name="feature_id" table="product_features" href_delete="product_features.delete?feature_id=`$subfeature.feature_id`&return_url=$r_url" delete_target_id="pagination_contents" header_text="{__("editing_product_feature")}: `$subfeature.description`" update_controller="product_features" no_table=true company_object=$subfeature no_popup=$show_in_popup}
                    {/if}
