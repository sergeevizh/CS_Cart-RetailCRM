        ...
        <div class="controls">
            ...
            {elseif $item.type == "G"}
                <div class="cm-combo-checkbox-group" id="{$html_id}">
                    {foreach from=$item.variants item="variant_name" key="variant_key"}
                        <label for="variant_{$item.name}_{$variant_key}" class="checkbox">
                            <input type="checkbox" class="cm-combo-checkbox" id="variant_{$item.name}_{$variant_key}" name="{$html_name}[]" value="{$variant_key}" {if $item.value[$variant_key] == "Y"}checked="checked"{/if} {if $disable_input}disabled="disabled"{/if}>
                            {$variant_name}
                        </label>
                    {foreachelse}
                        {__("no_items")}
