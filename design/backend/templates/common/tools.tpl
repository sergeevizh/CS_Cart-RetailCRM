...
<div class="btn-group {$tool_meta}">
    ...
    {if !$hide_actions}
        {if $tool_href|fn_check_view_permissions}
            <a class="{if $tool_override_meta}{$tool_override_meta}{else}btn{/if} cm-tooltip" {if $tool_id} id="{$tool_id}"{/if}{if $tool_href} href="{$tool_href|fn_url}"{/if}{if $tool_onclick} onclick="{$tool_onclick}; return false;"{/if} {if $title}title="{$title}"{/if}>
                {if $icon}
                    <i class="{$icon}"></i>
                {else}
                    <i class="icon-plus"></i>
